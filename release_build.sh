...
# Release Build Script for Rushie Client (Debian/Arch)
# Complete optimized build with all features enabled
...
echo "Configuring Extreme Release Build with OVERDOSE optimization..."
# Extreme optimization flags for maximum FPS
# -Ofast: Includes -O3 and -ffast-math (breaking some IEEE standards for speed)
# -march=native: Tune for your specific CPU
# -flto: Link Time Optimization
# -fomit-frame-pointer: Free up a register
# -funroll-loops: Speed up loops
# -fno-plt: Reduce shared library call overhead
EXTREME_FLAGS="-Ofast -march=native -flto -fomit-frame-pointer -funroll-loops -fno-plt -Wno-error=free-nonheap-object -Wno-error=stringop-overflow -Werror"

cmake -G Ninja \
      -DCMAKE_BUILD_TYPE=Release \
      -DCMAKE_CXX_FLAGS="$EXTREME_FLAGS" \
      -DCMAKE_C_FLAGS="$EXTREME_FLAGS" \
      -DCMAKE_EXE_LINKER_FLAGS="-flto -Wl,-O1 -Wl,--as-needed -Wl,--sort-common" \
      -DANTIBOT=ON \
      -DWEBSOCKETS=ON \
      -DDOWNLOAD_GTEST=ON \
      -DDISCORD=ON \
...
echo "----------------------------------------"
echo "Build finished! Executable is in build_release/DDNet"
echo "To run: ./build_release/DDNet"
